...
async function run() {
  const { data, error } = await supabase.from('registros').select('id, created_at, Data, Integrado, Alojamento').order('created_at', { ascending: true });
  ...
  if (toDelete.length > 0) {
    const { error: delError } = await supabase.from('registros').delete().in('id', toDelete);
