...
export const getExpectedPerformance = (idade: number, tipoLote?: 'Misto' | 'Fêmea' | 'Macho', pesoAloj?: number) => {
  ...
  for (let d = 0; d < idade; d++) {
    // Busca o ponto da curva pelo dia (1-indexed, então dia = d + 1)
    let point = curve.find(p => p.dia === (d + 1));
    if (!point) {
      point = curve[curve.length - 1]; // Se passar do limite, usa o último dia
    }
    
    totalConsumo += point.cmd;
    currentWeight += point.gpd; // Assume-se que o GPD também segue o dia da curva
  }
