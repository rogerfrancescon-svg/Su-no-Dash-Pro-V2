...
(async () => {
  ...
  fs.writeFileSync('test_final3.csv', Buffer.from('Data;Nome Integrado;Lote;Idade;Animais Alojados;Recomendação;Mortalidade;Comedouro;Peso Aloj;Pontuação Sanitária;Colaborador;Consumo\n01/01/2023;Teste8;1;10;1000;Ração X;0;Auto;30.5;Excelente;João;100', 'latin1'));
  
  const fileInput = await page.$('input[type="file"]');
  if (fileInput) {
    await fileInput.uploadFile(path.resolve('test_final3.csv'));
    await new Promise(r => setTimeout(r, 1000));
    
    // Read the textarea BEFORE importing
    const rawData = await page.evaluate(() => document.querySelector('textarea').value);
    console.log("Raw Data BEFORE import:", rawData);
    ...
    // Read logs quickly while they appear
